...
| `--routing "<codes>"` | Routing Codes | ITA path language (carriers, vias, operators) — see [references/ROUTING_CODES.md](references/ROUTING_CODES.md) | passed through, per slice |
| `--ext "<codes>"` | Extension Codes | ITA faring & filter codes (`f bc=`, `MAXDUR`, `-OVERNIGHTS`…) — see [references/ROUTING_CODES.md](references/ROUTING_CODES.md) | passed through, per slice |
...
| P5 | **agent skill** — NL → routing/extension codes via [references/ROUTING_CODES.md](references/ROUTING_CODES.md), wraps the CLI ([skill](SKILL.md)) | ✅ done |
